...
export const getWeatherData = async (
  ...
  const cacheKey = `${lat.toFixed(2)},${lon.toFixed(
    2
...
const generateMockWeather = (
  ...
  const conditionIndex = Math.floor(
    Math.pow(Math.random(), 1.5) * conditions.length
  );
...
export const calculateWeatherAlongRoute = async (
  ...
  for (let i = 1; i < coordinates.length; i++) {
    ...
    if (
      timeSinceLastCheckpoint >= timeIntervalMs ||
      distanceSinceLastCheckpoint >= distanceIntervalKm
    ) {
 ...
 */
export const evaluateWeatherRisk = (
  weather: WeatherPoint
): { isRisky: boolean; riskReason: string | null } => {
  ...
  for (const riskType of riskyConditions) {
    if (
      riskType.keywords.some((keyword) =>
